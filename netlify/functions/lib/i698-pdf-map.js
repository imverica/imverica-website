...
function sexFields(v){const s=clean(v,40).toLowerCase();
  if(/^m|male/.test(s))return{"P1_L10_Sex[0]":true,"P1_L10_Sex[1]":false};
  if(/^f|female/.test(s))return{"P1_L10_Sex[0]":false,"P1_L10_Sex[1]":true};
  return {};}

function maritalFields(v){const s=clean(v,80).toLowerCase();
  const all={"P1_15_MaritalStatus[0]":false,"P1_15_MaritalStatus[1]":false,"P1_15_MaritalStatus[2]":false,"P1_15_MaritalStatus[3]":false};
  if(/married|spouse|брак/.test(s))return{...all,"P1_15_MaritalStatus[1]":true};
  if(/single|never|холост/.test(s))return{...all,"P1_15_MaritalStatus[0]":true};
  if(/divorc|развед/.test(s))return{...all,"P1_15_MaritalStatus[2]":true};
  if(/widow|вдов/.test(s))return{...all,"P1_15_MaritalStatus[3]":true};
  if(/annul/.test(s))return{...all,"P1_15_MaritalStatus[0]":true};
  if(/separat/.test(s))return{...all,"P1_15_MaritalStatus[0]":true};
  return {};}

function i_698FieldValues(payload={}) {
  ...
  v["P1_9_DateOfBirth[0]"] = dateMdY(a.date_of_birth || a.dob || '');
  v["Pt1Line8_AlienNumber[0]"] = digits(a.alien_number || a.a_number, 9);
  v["P1_8_SSN[0]"] = digits(a.ssn || a.social_security_number, 9);
  v["Pt1Line5_StreetNumberName[0]"] = clean(a.mailing_address_line1 || a.current_address_line1 || a.address_line1, 80);
  v["Pt1Line5_CityOrTown[0]"] = clean(a.mailing_city || a.city, 60);
  v["Pt1Line5_State[0]"] = stateCode(a.mailing_state || a.state || '');
  v["Pt1Line5_ZipCode[0]"]   = digits(a.mailing_zip || a.zip_code, 10);
  v["P4_L5_EmailAddress[0]"]  = clean(a.email_address || a.email || c.email, 120);
...
module.exports = { i_698FieldValues };