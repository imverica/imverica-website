...
function sexFields(v){const s=clean(v,40).toLowerCase();
  if(/^m|male/.test(s))return{"Pt4Line3_Sex[0]":true,"Pt4Line3_Sex[1]":false};
  if(/^f|female/.test(s))return{"Pt4Line3_Sex[0]":false,"Pt4Line3_Sex[1]":true};
  return {};}

function maritalFields(v){const s=clean(v,80).toLowerCase();
  const all={"Pt3Line7_MaritalStatus[0]":false,"Pt3Line7_MaritalStatus[1]":false,"Pt3Line7_MaritalStatus[2]":false,"Pt3Line7_MaritalStatus[3]":false};
  if(/married|spouse|брак/.test(s))return{...all,"Pt3Line7_MaritalStatus[1]":true};
  if(/single|never|холост/.test(s))return{...all,"Pt3Line7_MaritalStatus[0]":true};
  if(/divorc|развед/.test(s))return{...all,"Pt3Line7_MaritalStatus[2]":true};
  if(/widow|вдов/.test(s))return{...all,"Pt3Line7_MaritalStatus[3]":true};
  if(/annul/.test(s))return{...all,"Pt3Line7_MaritalStatus[0]":true};
  if(/separat/.test(s))return{...all,"Pt3Line7_MaritalStatus[0]":true};
  return {};}
...
function i_360FieldValues(payload={}) {
  ...
  v["Pt10Line2_DateOfBirth[0]"] = dateMdY(a.date_of_birth || a.dob || '');
  v["Pt10Line5c1_AlienNumber[0]"] = digits(a.alien_number || a.a_number, 9);
  v["Pt3Line5_SSN[0]"] = digits(a.ssn || a.social_security_number, 9);
  v["USCISOnlineAcctNumber[0]"] = digits(a.uscis_online_account_number, 12);
  v["Pt10Line3_CountryOfBirth[0]"]  = clean(a.country_of_birth, 60);
  v["Pt3Line10_Passport[0]"] = clean(a.passport_number, 20);
  v["Pt3Line9_I94[0]"]     = clean(a.i94_number, 20);
  v["Pt1Line7_StreetNumberName[0]"] = clean(a.mailing_address_line1 || a.current_address_line1 || a.address_line1, 80);
  v["Pt1Line7_CityOrTown[0]"] = clean(a.mailing_city || a.city, 60);
  v["Pt1Line7_State[0]"] = stateCode(a.mailing_state || a.state || '');
  v["Pt1Line7_ZipCode[0]"]   = digits(a.mailing_zip || a.zip_code, 10);
...
module.exports = { i_360FieldValues };