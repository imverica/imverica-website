...
function i_361FieldValues(payload={}) {
  const a = payload.formAnswers || payload.answers || {};
  const c = payload.contact || {};
  const today = new Date().toISOString().slice(0,10);
  const v = {};
  v["P3_Line14d_DateofBirth[0]"] = dateMdY(a.date_of_birth || a.dob || '');
  v["P3_Line14e_AlienNumber[0]"] = digits(a.alien_number || a.a_number, 9);
  v["P1_Line5_SSN[0]"] = digits(a.ssn || a.social_security_number, 9);
  v["P2_Line4_CountryofBirth[0]"]  = clean(a.country_of_birth, 60);
  v["P4_Line4_MobilePhoneNumber[0]"]  = usPhone(a.daytime_phone || a.phone || c.phone);
  v["P5_Line5_EmailAddress[0]"]  = clean(a.email_address || a.email || c.email, 120);
  v["P5_Line1a_InterpretersFamilyName[0]"] = clean(a.interpreter_family_name, 60);
  v["P5_Line1b_InterpretersGivenName[0]"]  = clean(a.interpreter_given_name, 60);

  return Object.fromEntries(Object.entries(v).filter(([,val])=>val!==undefined&&val!==null&&val!==''));
}

module.exports = { i_361FieldValues };