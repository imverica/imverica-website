...
function g_845FieldValues(payload={}) {
  const a = payload.formAnswers || payload.answers || {};
  const c = payload.contact || {};
  const today = new Date().toISOString().slice(0,10);
  const v = {};
  v["P1_Line7_DateOfBirth[0]"] = dateMdY(a.date_of_birth || a.dob || '');
  v["P1_Line7_SSN[0]"] = digits(a.ssn || a.social_security_number, 9);
  v["P1_Line16a_DaytimeTelephone[0]"]  = usPhone(a.daytime_phone || a.phone || c.phone);

  return Object.fromEntries(Object.entries(v).filter(([,val])=>val!==undefined&&val!==null&&val!==''));
}

module.exports = { g_845FieldValues };