...
function i_407FieldValues(payload={}) {
  ...
  v["P1_Line2_USCISOnlineAccountNumber[0]"] = digits(a.uscis_online_account_number, 12);
  ...
  v["P2_Line5_InterEmailAddress[0]"]  = clean(a.email_address || a.email || c.email, 120);
  v["P4_Line4_DateofSignature[0]"] = dateMdY(a.applicant_signature_date);
  v["P2_Line1_InterpreterFamilyName[0]"] = clean(a.interpreter_family_name, 60);
  v["P2_Line1_InterpreterGivenName[0]"]  = clean(a.interpreter_given_name, 60);
  v["P2_Line2_InterpreterBusiness[0]"]    = clean(a.interpreter_org_name || a.interpreter_business_name, 80);
  v["P3_Line1_PreparerFamilyName[0]"] = clean(a.preparer_family_name, 60);
  v["P3_Line1_PreparerGivenName[0]"]  = clean(a.preparer_given_name, 60);
  v["P3_Line2_PreparerBusinessOrganization[0]"]    = clean(a.preparer_business_name, 80);

  return Object.fromEntries(Object.entries(v).filter(([,val])=>val!==undefined&&val!==null&&val!==''));
}

module.exports = { i_407FieldValues };