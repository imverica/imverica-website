...
function i_865FieldValues(payload={}) {
  const a = payload.formAnswers || payload.answers || {};
  const c = payload.contact || {};
  const v = {};
  v["P1_Line2_DateOfBirth[0]"] = dateMdY(a.date_of_birth || a.dob || '');
  v["P2_Line6_AlienNumber[0]"] = digits(a.alien_number || a.a_number, 9);
  v["P5_Line4_PreparersDaytimePhoneNumber[0]"]  = usPhone(a.daytime_phone || a.phone || c.phone);
  v["P3_Line4_MobileTelephoneNumber[0]"] = usPhone(a.mobile_phone || a.daytime_phone || c.phone);
  v["P3_Line5_EmailAddress[0]"]  = clean(a.email_address || a.email || c.email, 120);
  v["P3_Line6b_DateofSignature[0]"] = dateMdY(a.applicant_signature_date);
  v["P4_Line1a_InterpretersFamilyName[0]"] = clean(a.interpreter_family_name, 60);
  v["P4_Line1b_InterpretersGivenName[0]"]  = clean(a.interpreter_given_name, 60);
  v["P4_Line2_InterpretersBusinessName[0]"]    = clean(a.interpreter_org_name || a.interpreter_business_name, 80);
  v["P5_Line1a_PreparersFamilyName[0]"] = clean(a.preparer_family_name, 60);
  v["P5_Line1b_PreparersGivenName[0]"]  = clean(a.preparer_given_name, 60);
  v["P5_Line2_PreparersBusinessName[0]"]    = clean(a.preparer_business_name, 80);

  return Object.fromEntries(Object.entries(v).filter(([,val])=>val!==undefined&&val!==null&&val!==''));
}

module.exports = { i_865FieldValues };