...
function sexFields(v){const s=clean(v,40).toLowerCase();
  if(/^m|male/.test(s))return{"P1_Line8_Sex[0]":true,"P1_Line8_Sex[1]":false};
...
function i_526eFieldValues(payload={}) {
  ...
  const v = {};
  v["P3_Line33_DateOfBirth[0]"] = dateMdY(a.date_of_birth || a.dob || '');
  v["P2_Line1_AlienNumber[0]"] = digits(a.alien_number || a.a_number, 9);
  v["P2_Line3_SSN[0]"] = digits(a.ssn || a.social_security_number, 9);
  v["USCISELISAcctNumber[0]"] = digits(a.uscis_online_account_number, 12);
  v["P1_Line21_CityTownOfBirth[0]"] = clean(a.city_of_birth || a.place_of_birth_city, 60);
  v["Pt6Line1b_CountryofBirth[0]"]  = clean(a.country_of_birth, 60);
