...
function sexFields(v){const s=clean(v,40).toLowerCase();
  if(/^m|male/.test(s))return{"Part1_Sex[0]":true,"Part1_Sex[1]":false};
  if(/^f|female/.test(s))return{"Part1_Sex[0]":false,"Part1_Sex[1]":true};
  return {};}

function maritalFields(v){const s=clean(v,80).toLowerCase();
  const all={"Part1_MaritalStatus[0]":false,"Part1_MaritalStatus[1]":false,"Part1_MaritalStatus[2]":false,"Part1_MaritalStatus[3]":false,"Part1_MaritalStatus[4]":false};
  if(/married|spouse|брак/.test(s))return{...all,"Part1_MaritalStatus[1]":true};
  if(/single|never|холост/.test(s))return{...all,"Part1_MaritalStatus[0]":true};
  if(/divorc|развед/.test(s))return{...all,"Part1_MaritalStatus[3]":true};
  if(/widow|вдов/.test(s))return{...all,"Part1_MaritalStatus[3]":true};
  if(/annul/.test(s))return{...all,"Part1_MaritalStatus[0]":true};
  if(/separat/.test(s))return{...all,"Part1_MaritalStatus[4]":true};
  return {};}

function i_687FieldValues(payload={}) {
  const a = payload.formAnswers || payload.answers || {};
  const c = payload.contact || {};
  const today = new Date().toISOString().slice(0,10);
  const v = {};
  v["DateofBirth[0]"] = dateMdY(a.date_of_birth || a.dob || '');
  v["USCISAccountNumber[0]"] = digits(a.uscis_online_account_number, 12);
  v["CountryofBirth[0]"]  = clean(a.country_of_birth, 60);
  v["CountryofCitizenship[0]"]  = clean(a.country_of_citizenship, 60);
  v["PassportNumber[0]"] = clean(a.passport_number, 20);
  v["DaytimePhoneNumber[0]"]  = usPhone(a.daytime_phone || a.phone || c.phone);
  Object.assign(v, sexFields(a.sex || a.gender || ''));
  Object.assign(v, maritalFields(a.marital_status || ''));
  v["PrintPreparersFamilyName[0]"] = clean(a.preparer_family_name, 60);
  v["PrintPreparersGivenName[0]"]  = clean(a.preparer_given_name, 60);

  return Object.fromEntries(Object.entries(v).filter(([,val])=>val!==undefined&&val!==null&&val!==''));
}

module.exports = { i_687FieldValues };