...
function sexFields(v){const s=clean(v,40).toLowerCase();
  if(/^m|male/.test(s))return{"Part3_Sex[0]":true,"Part3_Sex[1]":false};
  if(/^f|female/.test(s))return{"Part3_Sex[0]":false,"Part3_Sex[1]":true};
  return {};}

function maritalFields(v){const s=clean(v,80).toLowerCase();
  const all={"Part1_MaritalStatus[0]":false,"Part1_MaritalStatus[1]":false,"Part1_MaritalStatus[2]":false,"Part1_MaritalStatus[3]":false,"Part2_MaritalStatus[0]":false,"Part2_MaritalStatus[1]":false,"Part2_MaritalStatus[2]":false,"Part2_MaritalStatus[3]":false};
  if(/married|spouse|брак/.test(s))return{...all,"Part2_MaritalStatus[1]":true};
  if(/single|never|холост/.test(s))return{...all,"Part2_MaritalStatus[0]":true};
  if(/divorc|развед/.test(s))return{...all,"Part2_MaritalStatus[2]":true};
  if(/widow|вдов/.test(s))return{...all,"Part2_MaritalStatus[3]":true};
  if(/annul/.test(s))return{...all,"Part1_MaritalStatus[0]":true};
  if(/separat/.test(s))return{...all,"Part1_MaritalStatus[0]":true};
  return {};}

function i_929FieldValues(payload={}) {
  ...
  v["CountryofCitizenshipNationality[0]"]  = clean(a.country_of_citizenship, 60);
...
module.exports = { i_929FieldValues };