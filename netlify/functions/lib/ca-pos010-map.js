...
function pos_010FieldValues(payload = {}) {
  ...
  // Caption (In-Pro-Per filer).
  v[C + 'AttyPartyInfo[0].Nmae[0]'] = plaintiff;  // "Nmae" is the form's verbatim (misspelled) field name
  v[C + 'AttyPartyInfo[0].TextField1[0]'] = [clean(pick(a, 'plaintiff_address_line1', 'mailing_address_line1'), 80), [clean(pick(a, 'plaintiff_city', 'mailing_city'), 60), clean(pick(a, 'plaintiff_state'), 20), clean(pick(a, 'plaintiff_zip'), 12)].filter(Boolean).join(', ')].filter(Boolean).join(', ');
  ...
  // Item 2 — documents served: Summons + Complaint (and UD-101/CM-010 noted).
