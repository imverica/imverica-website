...
function ar_11FieldValues(payload={}) {
  ...
  v["S2A_StreetNumberName[0]"] = clean(a.mailing_address_line1 || a.current_address_line1 || a.address_line1, 80);
  v["S2A_AptSteFlrNumber[0]"] = clean(a.mailing_address_line2 || a.address_unit, 10).replace(/^(?:apt|ste|fl|unit|#)\s*\.?\s*/i,'').slice(0,6);
  v["S2A_CityOrTown[0]"] = clean(a.mailing_city || a.city, 60);
  v["S2A_State[0]"] = stateCode(a.mailing_state || a.state || '');
  v["S2A_ZipCode[0]"]   = digits(a.mailing_zip || a.zip_code, 10);
