...
function i_730FieldValues(payload={}) {
  ...
  v["Pt1_Line1_MiddleName[0]"] = clean(a.applicant_middle_name || a.middle_name || '', 60);
  v["DateofBirth[0]"] = dateMdY(a.date_of_birth || a.dob || '');
  v["Part2_Iine9_SSN[0]"] = digits(a.ssn || a.social_security_number, 9);
  v["USCISOnlineAcctNumber[0]"] = digits(a.uscis_online_account_number, 12);
  v["CountryofBirth[0]"]  = clean(a.country_of_birth, 60);
  v["CountryofCitizenshipNationality[1]"]  = clean(a.country_of_citizenship, 60);
  v["Pt2_Line45_CountryIssuancePassport[0]"] = clean(a.passport_number, 20);
  v["P2_Line33_DatePassportExpire[0]"] = dateMdY(a.passport_expiration || '');
  v["Pt2_Line30_I94Number[0]"]     = clean(a.i94_number, 20);
  v["Pt1Line26_State[0]"] = stateCode(a.mailing_state || a.state || '');
  v["P8_Line4_DayTelephone[0]"]  = usPhone(a.daytime_phone || a.phone || c.phone);
  v["P8_Line5_MobileTelephone[0]"] = usPhone(a.mobile_phone || a.daytime_phone || c.phone);
  v["P8_Line6_PrepEmailAddress[0]"]  = clean(a.email_address || a.email || c.email, 120);
  v["P7_Line1a_InterpreterFamilyName[0]"] = clean(a.interpreter_family_name, 60);
  v["P7_Line1b_InterpreterGivenName[0]"]  = clean(a.interpreter_given_name, 60);
  v["P7_Line2_InterpreterBusiness[0]"]    = clean(a.interpreter_org_name || a.interpreter_business_name, 80);

  return Object.fromEntries(Object.entries(v).filter(([,val])=>val!==undefined&&val!==null&&val!==''));
}

module.exports = { i_730FieldValues };