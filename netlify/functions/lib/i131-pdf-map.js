...
function sexFields(v){const s=clean(v,40).toLowerCase();
  if(/^m|male/.test(s))return{"P4_Line6a_Gender[0]":true,"Part2_Line8_Gender[0]":false};
  if(/^f|female/.test(s))return{"P4_Line6a_Gender[0]":false,"Part2_Line8_Gender[0]":true};
  return {};}
...
function i_131FieldValues(payload={}) {
  ...
  const v = {};
  v["P4_Line6a_DOB[0]"] = dateMdY(a.date_of_birth || a.dob || '');
  v["P2_Line23_AlienNumber[0]"] = digits(a.alien_number || a.a_number, 9);
  v["Part2_Line10_SSN[0]"] = digits(a.ssn || a.social_security_number, 9);
  v["Part2_Line11_USCISOnlineAcctNumber[0]"] = digits(a.uscis_online_account_number, 12);
  v["P2_Line19_CountryOfBirth[0]"]  = clean(a.country_of_birth, 60);
  v["P4_Line6a_CountryofBirthCitizenship[0]"]  = clean(a.country_of_citizenship, 60);
  v["Part12_Line5_Email[0]"]  = clean(a.email_address || a.email || c.email, 120);
  v["Part12_Line6_DateofSignature[0]"] = dateMdY(today);
  Object.assign(v, sexFields(a.sex || a.gender || ''));
  v["Part11_Line1_InterpreterFamilyName[0]"] = clean(a.interpreter_family_name, 60);
  v["Part11_Line1_InterpreterGivenName[0]"]  = clean(a.interpreter_given_name, 60);
